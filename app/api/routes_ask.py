...
async def ask_household(req: AskQueryRequest):
    """
    Execute natural language RAG query over household memory.
    Returns grounded answer with source attribution badges, confidence, and suggested follow-ups.
    """
    if not req.query or not req.query.strip():
        raise HTTPException(status_code=400, detail="Query string cannot be empty")

    products = store.get_all()
    result = answer_household_query(req.query, products)
